;------------------------------MAIN--------------------------------
	...
	JSR	DIV			;Multiply R1 and R2. Result is in R0.
...
MOD
	...
	ADD R0, R0, R1	;Set R0 to R1

	ADD	R1, R1, #0	;Check R1 for 0
	BRZ	MD_END		;Restore and return if Zero
	ADD	R2, R2, #0	;Check R2 for 0
	BRZ	MD_ZERO		;R1 set to -1 then restore and return if Zero
